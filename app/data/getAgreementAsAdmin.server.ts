...
const getAgreementAsAdmin = (
  ...
  return verifyAdminUser(userId)
    ...
    .then(({ execute, destroy }) =>
      execute(
        `SELECT c.type, c.userId, a.uuid, a.amount, i.name, i.email, e.id, d.label, d.value
FROM agreement a
...
WHERE a.uuid = ?`,
        ...
        const status = id
          ? await getEversign()
              ...
              .catch(() => 0)
          : 0;
        const creator = await import("@clerk/clerk-sdk-node").then((clerk) => clerk.users.getUser(agreement.userId));
        return {
            type: FUNDRAISE_TYPES[agreement.type].id,
            uuid: agreement.uuid,
            creatorName: `${creator.firstName} ${creator.lastName}`,
            creatorEmail: creator.emailAddresses.find(e => e.id === creator.primaryEmailAddressId)?.emailAddress,
