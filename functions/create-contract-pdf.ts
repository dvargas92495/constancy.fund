...
export const handler = ({ uuid }: { uuid: string }) => {
  ...
  const detailsData: Record<string, string | Date> = {
    ...
    ...Object.fromEntries(details.map(({ label, value }) => [label, value])),
  };*/const detailsData: Record<string, string | Date> = {};const data = contentByType.isa as ContractData;
